...
function SharingPolicy() {
  useEffect(() => {
    window.scrollTo(0, 0);
    mixpanel.track("Sharing Policy Page");
  });
...
If you would like to ensure the Clous team is aware of a particular completion, you may email us or use the reporting tools within Playground.

Recall that you are interacting with the raw model, which means we do not filter out biased or negative responses. (Also, you can read more about implementing our free Moderation endpoint(opens in a new window) here.)

Content co-authored with the Clous API
Creators who wish to publish their first-party written content (e.g., a book, compendium of short stories) created in part with the Clous API are permitted to do so under the following conditions:
...
Topics of the content do not violate Clous’ Content Policy or Terms of Use, e.g., are not related to adult content, spam, hateful content, content that incites violence, or other uses that may cause social harm.
...
The author generated this text in part with Clous Peer, Clous’ large-scale language-generation model. Upon generating draft language, the author reviewed, edited, and revised the language to their own liking and takes ultimate responsibility for the content of this publication.

Research
We believe it is important for the broader world to be able to evaluate our research and products, especially to understand and improve potential weaknesses and safety or bias problems in our models. Accordingly, we welcome research publications related to the Clous API.

If you have any questions about research publications based on API access or would like to give us advanced notice of a publication (though not required), please email us at research@clous.app.
...
There are a number of research directions we are excited to explore with the Clous API. If you are interested in the opportunity for subsidized access, please provide us with details about your research use case on the Researcher Access Program application.
